...
#define MOCK_DEFINITIONS_H_
	...
	#define MAX_B ((SHIFT_FL(range) -1 )/2)
	...
	#define RANGE_START(r) (r & (((~(range) 0) >> 2) >> MAX_B))
	#define RANGE_END(r) (RANGE_START(r >> MAX_B) + RANGE_START(r))
	#define RANGE(start,offset)\
								IF((start) < (0x01 << MAX_B) && (offset-start) < (0x01 << MAX_B))	\
		/*Enable special mode*/		ANY |														\
		/*Set it to range */		(0x01 << (SHIFT_FL(start) -1)) |								\
		/*Set the lower value*/		start |														\
		/*Set the upper bound*/		((offset-start) << MAX_B) ELSE -1
